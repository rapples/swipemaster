...
function generateGrid() {
  ...
  gameDiv.innerHTML = "";
  dots = [];
  const grid = levels[currentLevel].slice();
  
  for (let i = 0; i < grid.length; i++) {
    const dot = document.createElement("div");
    dot.classList.add("dot");
    dot.setAttribute("data-index", i);
    dot.innerText = grid[i];
...
if (remainingMatches === 0) {
  ...
  const missingMatches = findMissingMatches(unconnectedDots.map(dot => dot.innerText));
  
  // Highlight missing matches in red
  missingMatches.forEach(match => {
    const matchingDots = dots.filter(dot => dot.innerText === match && !dot.classList.contains("connected"));
    matchingDots.forEach(dot => {
...
function levelUp() {
  ...
  if (levelIndex < levels.length) {
    generateGrid(levels[levelIndex]);
  } else {
    // End of game
